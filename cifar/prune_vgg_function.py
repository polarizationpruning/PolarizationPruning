...
from models.vgg import vgg16, vgg16_linear
...
def prune_vgg(num_classes: int, model, pruning_strategy, cuda=True, dataparallel=False):
...
if __name__ == '__main__':
    test_model = vgg16_linear(10).cuda()
    ...
    pruned_model, cfg = prune_vgg(10, test_model,
                                  pruning_strategy="fixed",
                                  dataparallel=False)

    demo_input = torch.rand(3, 3, 32, 32).cuda()

    original_output = test_model(demo_input)
    pruned_output = pruned_model(demo_input)

    assert (original_output - pruned_output).abs().mean().item() < 1e-7
    pass
