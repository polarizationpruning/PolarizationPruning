...
parser.add_argument('--decay-epoch', type=float, nargs='*', default=[20, 60, 150],
                    help="the epoch to decay the learning rate (default 20, 60, 150)")
parser.add_argument('--gammas', type=float, nargs='+', default=[0.5, 0.5, 0.4],
...
def save_checkpoint(state, is_best, filepath):
    ...
    torch.save(state, os.path.join(filepath, 'ft_checkpoint.pth.tar'))
    if is_best:
        shutil.copyfile(os.path.join(filepath, 'ft_checkpoint.pth.tar'), os.path.join(filepath, 'ft_model_best.pth.tar'))
